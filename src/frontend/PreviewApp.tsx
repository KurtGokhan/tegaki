...
export function PreviewApp() {
  ...
  const handleFileUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      try {
        const info = parseFont(reader.result as ArrayBuffer);
        setFontInfo(info);
  ...
  // Animation loop
  useEffect(() => {
    if (!animPlaying || activeStage !== 'animation') return;
    ...
    <div className="flex h-screen bg-gray-50 text-gray-900">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className={`h-[44px] ${(activeStage === 'animation' || activeStage === 'final') && result ? '' : 'invisible'}`}>
          {result && (
            <AnimationControls result={result} time={animTime} setTime={setAnimTime} playing={animPlaying} setPlaying={setAnimPlaying} />
          )}
        </div>
